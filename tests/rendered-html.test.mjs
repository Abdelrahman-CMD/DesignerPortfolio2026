...
test("server-renders the complete portfolio homepage", async () => {
  ...
  assert.match(html, /hero-profile-window-v1\.webp/);
  assert.match(html, /hero-profile-foreground-v1\.webp/);
  assert.match(html, /brain-stage-lumi\.png/);
  assert.equal((html.match(/brain-stage-zone-(curiosity|connections|structure|source|direction)\.png/g) ?? []).length, 5);
...
test("uses bounded raster assets on the homepage and case pages", async () => {
  ...
  assert.match(homeSource, /\/about\/hero-profile-window-v1\.webp/);
  assert.match(homeSource, /\/about\/hero-profile-foreground-v1\.webp/);
  assert.match(homeSource, /\/about\/brain-stage-lumi\.png/);
  assert.match(homeSource, /\/about\/brain-stage-zone-\$\{zone\.id\}\.png/);
